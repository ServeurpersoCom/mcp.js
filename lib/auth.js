...
const { enabled, clientSecret } = config.auth;
const AUTH_ENABLED = Boolean(enabled);

if (AUTH_ENABLED && (typeof clientSecret !== 'string' || clientSecret.length === 0)) {
	console.error('[Auth] auth.enabled=true but auth.clientSecret is empty, aborting');
	process.exit(1);
}

// Pre encode the secret once to avoid per request allocation
const SECRET_BUF = AUTH_ENABLED ? Buffer.from(clientSecret, 'utf8') : null;
