...
async function tool_view(args = {}) {
	...
	if (mimeLine.toLowerCase().includes('binary')) {
		if (mimeType && supportedImageTypes.has(mimeType)) {
			...
			const base64Result = await bashExec(base64Script, config.bash.fileLimitBytes);

			if (base64Result.exitCode !== 0) {
				return textResult(base64Result.stdout + `❌ Error encoding image ${filepath}`, true);
			}

			if (base64Result.truncated) {
				return textResult(`❌ Image ${filepath} exceeds ${config.bash.fileLimitBytes} bytes`, true);
			}
	...
	const catResult = await bashExec(catScript, config.bash.fileLimitBytes);

	if (catResult.exitCode !== 0) {
		return textResult(catResult.stdout + `❌ Error reading file ${filepath}`, true);
	}

	if (catResult.truncated) {
		return textResult(`❌ File ${filepath} exceeds ${config.bash.fileLimitBytes} bytes`, true);
	}
...
async function tool_str_replace(args = {}) {
	...
	const readResult = await bashExec(readScript, config.bash.fileLimitBytes);
